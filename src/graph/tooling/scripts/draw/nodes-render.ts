...
export function updateNodePosition(node: GraphDataNode, svgElement: Element, nodeCanvasId: string) {
	(
		d3
			.select(svgElement)
			.selectAll(`#${nodeCanvasId}-group-${toHTMLToken(node.id)}`) as d3.Selection<
			d3.BaseType,
			GraphDataNode,
			Element,
			unknown
...
export function renderNodes(
	...
	const nodeSelection = d3
		...
		.join('g')
		.attr('class', 'nodes')
		.attr('id', (n) => `${nodeCanvasId}-group-${toHTMLToken(n.id)}`)
	...
	rectangle
		...
		.on('mouseover', function (event, data) {
			...
			data.outgoingLinks.forEach((link) => {
				d3.select(`#${canvasId}-line-${toHTMLToken(link.id)}`).style(
					'stroke-width',
	...
	nodes.forEach((node) => {
		...
		renderNodes(node.members, element, drawSettings, OnNodeClick, nodeCanvasId, canvasId);
	});
}
